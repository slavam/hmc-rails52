class Energy < Prawn::Document
  ...
  def initialize(temperatures, year, month, chief, responsible)
    ...
    bounding_box([250, y_pos], width: bounds.width-250) do
      ...
      text "Заместителю директора филиала -
            главному инженеру
            АО \"ИНФРАСТРУКТУРНЫЕ ПРОЕКТЫ\"-
            \"Зуевская ТЭС\"
            С.В. Проскурникову", leading: 3
            # А.В. Кошельникову", leading: 3
    end
    text "О предоставлении информации"
    move_down 20
    @last_day = Time.days_in_month(month.to_i, year.to_i)
    text "ФГБУ \"УГМС по ДНР\" предоставляет информацию о средней за сутки температуре воздуха в г. Зугрэсе за период 01-#{@last_day} #{Bulletin::MONTH_NAME2[month.to_i]} #{year} г. по дням по данным репрезентативных метеорологических станций:", indent_paragraphs: 40, leading: 2
    case @last_day
      when 28
        @lines = 14
      when 31
