class AmvrosievkaTemp < Prawn::Document
  def initialize(temperatures, year, month, chief, responsible)
    ...
    bounding_box([0, y_pos], width: 300, leading: 3) do
      text "____________________  № 06/________"
      text "На № 04/3268           от 11.10.2021"
    end
    
    bounding_box([250, y_pos], width: bounds.width-250) do
      text "Начальнику
            ГПСО г. Амвросиевка МЧС ДНР

            полковнику службы
            гражданской защиты
            Д.С. Анищенко", leading: 3
    end
    text "О предоставлении информации"
    move_down 20
    @last_day = Time.days_in_month(month.to_i, year.to_i)
    text "Гидрометцентр МЧС ДНР предоставляет информацию о средней за сутки температуре воздуха в г. Амвросиевке за период 01-#{@last_day} #{Bulletin::MONTH_NAME2[month.to_i]} #{year} г. по данным метеорологической станции Амвросиевка:", indent_paragraphs: 40, leading: 2
    case @last_day
      when 28
        @lines = 14
      when 31
