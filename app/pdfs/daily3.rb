class Daily3 < Prawn::Document
  ...
  def initialize(bulletin, num_pages)
    ...
    bounding_box([0, cursor], :width => bounds.width, :height => 20) do
      text @bulletin.header_period_plus1, align: :center
    end
    font "OpenSans"
    bounding_box([0, cursor], :width => bounds.width) do
      text "в городе Донецке", style: :bold
      text @bulletin.forecast_sea_period.split('@')[0]
    end
    bounding_box([0, cursor], :width => bounds.width) do
      text "в Донецкой Народной Республике", style: :bold
      text @bulletin.forecast_period.split('@')[0]
    end
    move_down 10
    font "OpenSans", style: :bold
    bounding_box([0, cursor], :width => bounds.width, :height => 20) do
