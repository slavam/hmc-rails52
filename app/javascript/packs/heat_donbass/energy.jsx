...
import EnergyForm from './energy_form';
...
export default class Energy extends React.Component{
  ...
  handleFormSubmit(year, month){
    ...
    $.ajax({
      type: 'GET',
      dataType: 'json',
      url: "/synoptic_observations/energy?year="+year+"&month="+month
      }).done((data) => {
  ...
  render(){
    ...
    return(
      <div>
        <EnergyForm year={this.state.year} month={this.state.month} onFormSubmit={this.handleFormSubmit} />
        <h4>Средняя за сутки температура воздуха (°С) с 01 по {endDate} года в г. Зугрэс</h4>
        <br/>
...
$(() => {
  ...
  if(node) {
    ...
    ReactDOM.render(
      <Energy temperatures={temperatures} year={year} month={month} />,
