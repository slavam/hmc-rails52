...
const LastData = ({observations, tlgType, stations, onClickDelete}) => {
  ...
  observations.forEach((o) => {
    ...
    else if(tlgType === 'duty' || tlgType === 'duty_synoptic'){
      rows.push(<tr key={o.id}><td>{o.obs_date}</td><td>{o.created_at.toLocaleString().replace('T',' ').slice(0,19)}</td><td>{stations[o.station_id]}</td><td>{o.description}</td><td><button id={o.id} onClick={o => onClickDelete(o)}>Удалить</button></td></tr>)
    }else
